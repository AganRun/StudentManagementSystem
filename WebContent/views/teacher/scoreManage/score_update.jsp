	...
	<body onload="startTime()">
		...
		<div class="nav-down">
			...
			<div class="rightcon">
				...
				<form class="form-horizontal" role="form" id="updateScoreForm" action="${pageContext.request.contextPath}/teachScoreManage/saveScore">
				
					<div style="height:100px"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label"><font size="4px">修改成绩</font></label>
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<select id="score_stu_no" name="score_stu_no" class="form-control">
								<option value="${requestScope.updateScoreData.getScore_stu_no() }">${requestScope.updateScoreData.getScore_stu_no() }</option>
								<c:forEach items="${requestScope.stuList }" var="stu">
									<c:if test="${requestScope.updateScoreData.getScore_stu_no() != stu.getStu_no() }">
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<select name="score_subject_no" class="form-control">
								<option value="${requestScope.updateScoreData.getScore_subject_no() }">${requestScope.updateScoreData.getSubject_name() }</option>
								<c:forEach items="${requestScope.subjectList }" var="subject">
									<c:if test="${requestScope.updateScoreData.getScore_subject_no() != subject.getSubject_no() }">
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<input class="form-control" name="score_num" value="${requestScope.updateScoreData.getScore_num() }" type="text" >
						</div>
					</div>
					
					
					<input type="hidden" value="${requestScope.updateScoreData.getScore_no() }" name="score_no">
