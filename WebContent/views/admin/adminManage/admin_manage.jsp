	...
	<body onload="startTime()">
		...
		<div class="nav-down">
			...
			<div class="rightcon" >
				...
				<form class="form-horizontal" role="form" id="addForm" action="${pageContext.request.contextPath}/admin/addAdmin">
				
					<div class="form-group">
						<label class="col-sm-2 control-label"><font size="4px">管理员添加</font></label>
					...
					<hr style="border: dashed 1px aqua;" />
					
					<div class="form-group">
						<label class="col-sm-2 control-label"><font size="4px">已有管理员列表</font></label>
						<div class="col-sm-6"></div>
					</div>
					...
					<table class="table table-hover" style="width:90%;" align="center">
						...
						<tbody>
							...
							<tr>
								<td><c:out value="${admin.getAdmin_no() }"/></td>
								<td><c:out value="${admin.getAdmin_name() }"/></td>
								<td><c:out value="${admin.getAdmin_password() }"/></td>
								<td><c:out value="${admin.getAdmin_right() }"/></td>
								<td><c:out value="${admin.getAdmin_addtime() }"/></td>
								<td>
									<!-- 自己不能删除自己 -->
									<c:if test="${admin.getAdmin_no() != sessionScope.adminSession.getAdmin_no() }"><a href="${pageContext.request.contextPath}/admin/deleteAdmin?admin_no=${admin.getAdmin_no() }"><font color="white">删除</font></a></c:if>
									<c:if test="${admin.getAdmin_no() == sessionScope.adminSession.getAdmin_no() }"> </c:if>
								</td>
							</tr>
							</c:forEach>
